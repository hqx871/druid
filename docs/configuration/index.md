...
|`druid.query.scheduler.mergeBuffer.strategy`|Merge buffer schedule strategy. With `arbitrary` strategy, every query have an opportunity to take required buffer. On the contrary, `lane` strategy will limit laning query to use partial resource according to lane capacity. |`arbitrary`|
